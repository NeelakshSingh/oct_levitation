...
class ZRPSingleDipoleController(ControlSessionNodeBase):
    ...
    def post_init(self):
        ...
        self.control_input_publisher = rospy.Publisher("/z_rp_control_single_dipole/control_input",
        ...
        self.error_state_pub = rospy.Publisher("/z_rp_control_single_dipole/error_states",
                                                         VectorStamped, queue_size=1)
        
        self.ref_actual_pub = rospy.Publisher("/z_rp_control_single_dipole/ref_actual_values",
                                                         VectorStamped, queue_size=1)
        
        self.control_gain_publisher = rospy.Publisher("/z_rp_control_single_dipole/control_gains",
        ...
        if self.publish_jma_condition:
            self.jma_condition_pub = rospy.Publisher("/z_rp_control_single_dipole/jma_condition",
                                                     VectorStamped, queue_size=1)
        ...
        Az_d_norm, Bz_d_norm, Cz_d_norm, Dz_d_norm, dt = signal.cont2discrete((Az_norm, Bz_norm, Cz_norm, 0), dt=1/self.control_rate,
                                                  method='zoh')
        ...
        rospy.loginfo(f"Control gains for Fz: {self.K_z}, Tx: {self.K_phi}, Ty: {self.K_theta}")
    ...
    def simplified_allocation(self, tf_msg: TransformStamped, Tau_x: float, Tau_y: float, F_z: float) -> np.ndarray:
    ...
    def callback_control_logic(self, tf_msg: TransformStamped):
        ...
        self.ref_actual_msg.vector = [z_com, ref_z, phi, phi_ref, theta, theta_ref]
        ...
        self.error_state_msg.vector = np.concatenate((z_error.flatten(), phi_error.flatten(), theta_error.flatten()))
        self.error_state_pub.publish(self.error_state_msg)
        self.control_input_message.vector = [Tau_x, Tau_y, F_z]

        com_wrench_des = np.array([Tau_x, Tau_y, 0.0, 0.0, 0.0, F_z])
        ...
        des_currents = self.simplified_allocation(tf_msg, Tau_x, Tau_y, F_z)

        self.desired_currents_msg.des_currents_reg = des_currents

if __name__=="__main__":
    controller = ZRPSingleDipoleController()
    rospy.spin()